...
Il \glossario{repository} segue la struttura indicata in \hyperref[fig:repo_sorgenti_documenti]{Figura \ref{fig:repo_sorgenti_documenti}}.
\begin{figure}[H]
    \dirtree{%
    ...
    \caption{Struttura \glossario{repository} SorgentiDocumentazione.}
...
\begin{enumerate}
    ...
    \item \texttt{<file workflows>} sono dei file che definiscono le automazioni associate al \glossario{repository} e vengono denominati usando la sintassi \glossario{Pascal case}.
...
\subparagraph{RTB}
La cartella \glossario{RTB} ha la struttura indicata in \hyperref[fig:repo_sorgenti_documenti_RTB]{Figura \ref{fig:repo_sorgenti_documenti_RTB}}.
\begin{figure}[H]
    \dirtree{%
        .1 RTB.
            .2 DocumentiInterni.
    ...
    \caption{Struttura cartella \glossario{RTB}.}
...
Tale tabella se il \glossario{repository} fosse gestito in modo impeccabile sarebbe una copia dei dati già registrati nella stessa.
Tuttavia il metodo di lavoro del gruppo è in continua evoluzione quindi possono accadere modifiche distruttive che eliminano le versioni dei \glossario{configuration item} apparteneti alla \glossario{baseline} precedente.
Il registro delle modifiche è quindi uno strumento a supporto del controllo di configurazione ed è più "attendibile" rispetto alla storia del \glossario{repository} \glossario{Git}.
...
\item Modificare il documento assicurandosi di soddisfare la \glossario{check list} e le informazioni aggiuntive date dal verificatore che ha rifiutato la modifica.
Questa operazione deve avvenire seguendo il processo che regola l'implementazione della tipologia di \glossario{configuration item}.
