...
Questo approccio iterativo e continuo consente di monitorare e ottimizzare costantemente i processi adottati e i prodotti realizzati, assicurando che siano conformi agli standard richiesti e orientati al miglioramento continuo.
...
\begin{itemize}
    ...
    È essenziale pianificare le attività che saranno svolte durante il \glossario{ciclo di vita} del progetto, stabilendo le \glossario{metriche} da monitorare e i criteri di accettazione.
    Vengono quindi stabilite le azioni da intraprendere, le risorse necessarie e le tempistiche per l'implementazione. 
    Una pianificazione accurata è essenziale affinché le fasi successive possano portare a risultati concreti e misurabili.
    ...
    In questa fase è fondamentale documentare ogni passaggio e osservare attentamente l'andamento delle attività, così da raccogliere informazioni utili per la fase successiva.
    \item \textbf{Check}: Questa fase consiste nel confrontare i risultati ottenuti con gli obiettivi di qualità definiti.
    Si verificano i dati raccolti durante la fase "Do" per determinare se i processi e i risultati sono allineati con gli standard di qualità e con i requisiti.
    Questo momento di analisi è cruciale, perché permette di individuare eventuali discrepanze tra i risultati attesi e quelli reali, evidenziando sia i successi sia gli aspetti ancora da ottimizzare.
...
Ogni metrica è identificata in modo univoco seguendo la notazione: \textit{M}.[\textit{Tipo}].[\textit{Abbreviazione Nome}].
Dove:
\begin{itemize}
...
L'elenco delle \glossario{metriche} di qualità da adottare per il progetto è dettagliato secondo la struttura definita e copre diverse aree rilevanti per il processo di accertamento della qualità. Le \glossario{metriche} includono:
...
\begin{itemize}
    ...
    \item \textbf{Formula}: $BAC \times (\% Lavoro Pianificato)$, dove $BAC$ indica \glossario{Budget at Completion} cioè il budget previsto per la realizzazione del progetto
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Formula}: $BAC \times (\% Lavoro Completato)$, dove $BAC$ indica \glossario{Budget at Completion} cioè il budget previsto per la realizzazione del progetto
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Formula}: $AC + (BAC - EV)$, (costo sostenuto + stima costi da sostenere), dove $BAC$ indica il \glossario{Budget at Completion} cioè il budget previsto per la realizzazione del progetto
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Formula}: $\frac{M_s}{M_t} \times 100$, dove $M_s$ indica il numero di \glossario{metriche} soddisfatte mentre $M_t$ il numero totale delle \glossario{metriche} analizzate 
\end{itemize}
