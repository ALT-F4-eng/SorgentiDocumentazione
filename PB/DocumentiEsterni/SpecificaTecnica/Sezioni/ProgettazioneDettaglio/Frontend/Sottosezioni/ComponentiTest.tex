...
\textbf{Elementi chiave}:
\begin{itemize}
    \item \textbf{New Name Label}: label che permette di inserire il nuovo nome da assegnare al test;
...
\textbf{Elementi chiave}:
\begin{itemize}
    \item \textbf{Name Text}: sezione testuale contenente il nome del test caricato, nel caso non sia temporaneo;
    \item \textbf{Temporary Text}: sezione testuale contenente il messaggio che informa l'utente che il test caricato è temporaneo;
    \item \textbf{Summary Section}: sezione di visualizzazione dei dati sintetici del test, realizzata tramite il sottocomponente \textbf{TestIndex};
    \item \textbf{ComparisonButton}: pulsante che permette di richiedere il confronto tra il test corrente e un altro test salvato, il confronto viene poi effettuato tramite il sottocomponente \textbf{TestComparisonPage};
    \item \textbf{SaveButton}: pulsante che permette il salvataggio delle modifiche non salvate realizzate sul test caricato; se il test caricato è temporaneo, viene successivamente richiesto di assegnargli un nome tramite il sottocomponente \textbf{TestNameForm};
    \item \textbf{Graph Section}: sezione di visualizzazione dei vari tipi di grafico relativi ai risultati del test, realizzata tramite il sottocomponente \textbf{TestPageView};
...
\begin{itemize}
    ...
    \item \textbf{Legend}: legenda che descrive il significato di ogni elemento del diagramma;
\end{itemize}
\textbf{Input}:
\begin{itemize}
...
\end{itemize}
\subsubsection{TestSelectionComparison}
\textbf{Componente padre}:
\begin{itemize}
    \item TestPage;
\end{itemize}
\textbf{Elementi chiave}:
\begin{itemize}
    \item \textbf{SelectCompareButton}: pulsante per selezionare un test salvato con cui eseguire il confronto con il test attualmente visualizzato, la conferma o l'annullamento della selezione viene poi effettuata tramite il sottocomponente \textbf{ConfirmComponent};
\end{itemize}
    \textbf{Input}:
\begin{itemize}
    \item \textbf{dataset}: dataset associato al test;
...
\end{itemize}
\subsubsection{ComparisonPageView}  
\textbf{Componente padre}:
\begin{itemize}
    \item TestComparisonPage;
\end{itemize}
...
\begin{itemize}
    \item \textbf{ChangePage}: evento emesso quando l'utente interagisce con `PageNavigation`;
\end{itemize}
\subsubsection{ComparisonListView}
\textbf{Componente padre}:
\begin{itemize}
    \item ComparisonPageView;
\end{itemize}
...
\end{itemize}
\subsubsection{ComparisonElement}
\textbf{Componente padre}:
\begin{itemize}
