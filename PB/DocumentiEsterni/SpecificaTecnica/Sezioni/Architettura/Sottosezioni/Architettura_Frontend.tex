...
\glossario{Angular} adotta il pattern architetturale Model-View-ViewModel (MVVM), un pattern architetturale che separa le responsabilità dell'interfaccia utente (UI) dalla logica di business di un'applicazione.
In \glossario{Angular}, l'MVVM è implementato attraverso i componenti, che fungono da ViewModel, gestendo il legame tra il Model (i dati e la logica dell'applicazione) e la View (l'interfaccia utente).
...
\begin{itemize}
    \item \textbf{Model}: Si riferisce ai dati dell'applicazione e alla logica di business, come la validazione dei dati o l'elaborazione delle operazioni. 
    ...
    Non contiene logica di business, ma si collega al ViewModel, che fornisce i dati e le operazioni necessari per visualizzare e interagire con le informazioni.
    ...
    Quando i dati del Model cambiano a seguito dell'elaborazione della logica di business, la View si aggiorna automaticamente grazie al data binding bidirezionale con il ViewModel;
...
\begin{itemize}
    ...
    \item \textbf{Directive e template engine}: \glossario{Angular} estende l'HTML con directive personalizzate che permettono di manipolare il DOM in modo dinamico.
    ...
    \item \textbf{RxJS e programmazione reattiva}: \glossario{Angular} utilizza RxJS (Reactive Extensions for \glossario{Javascript}) per gestire lo stato e la comunicazione tra componenti in modo asincrono e reattivo.
...
\begin{itemize}
    \item \textbf{core}: contiene modelli e logica di business all'interno delle sottocartelle:
    \begin{itemize}
        \item \textbf{models}: interfacce e classi che rappresentano i dati e le entità principali dell'applicazione;
        \item \textbf{services}: gestiscono la logica di business e le chiamate API;
    ...
    \begin{itemize}
        ...
        \item \textbf{pages}: contiene le pagine principali della feature, ogni pagina è un componente a sé stante e funge da ViewModel nel pattern MVVM.
    \end{itemize}
