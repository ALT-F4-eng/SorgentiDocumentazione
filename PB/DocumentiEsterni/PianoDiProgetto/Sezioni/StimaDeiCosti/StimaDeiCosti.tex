\section{Stima dei costi}

\label{sec:stima_costi}
A seguito dello studio del \glossario{capitolato} Artifical QI e alla realizzazione dell'analisi dei rischi, 
il gruppo ha realizzato una stima preliminare dei costi per la realizzazione del progetto.
La stima verrà rivalutata, se necessario, dal responsabile di progetto al termine di uno \glossario{sprint} di lavoro e potrà essere modificata.

\begin{table}[H]
    \centering
    \resizebox{\textwidth}{!}{
    \begin{tabular}{| l | l | l | l | l |}
        \hline
            \textbf{Ruolo} &  
            \textbf{Ore per ruolo} & 
            \textbf{Ore per membro} &
            \textbf{Costo orario} &
            \textbf{Costo totale} \\
        \hline
        \hline
            Responsabile & 44 & 7 & 30€ & 1320€ \\
        \hline
            Amministratore & 54 & 9 & 20€ & 1080€ \\
        \hline 
            Analista & 78 & 13 & 25€ & 1950€ \\
        \hline 
            Progettista & 96 & 16 & 25€ & 2400€ \\
        \hline 
            Programmatore & 150 & 25 & 15€ & 2250€ \\
        \hline 
            Verificatore & 150 & 25 & 15€ & 2250€ \\
        \hline 
        \textbf{Totale} & \textbf{572} & \textbf{95} & \textbf{N/A} &\textbf{11250€}\\ 
        \hline  
    \end{tabular}}
    \caption{Ripartizione ore e costi.}
    \label{tab:stima_costi_totali} 
\end{table}

In seguito viene mostrato un diagramma a torta in cui viene indicata la ripartizione dei ruoli in percentuale rispetto le ore totali a disposizione.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \pie{7.69/Responsabile, 9.44/Amministratore, 13.63/Analista, 16.78/Progettista, 26.23/Programmatore, 26.23/Verificatore}
    \end{tikzpicture}
    \caption{Ripartizione ore e ruoli.}
    \label{fig:pie_ruoli_totali}
\end{figure}